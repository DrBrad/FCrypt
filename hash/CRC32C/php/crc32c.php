<?php
    class CRC32C {
        ...
        private $T8_0_START = 0 * 256;
        private $T8_1_START = 1 * 256;
        private $T8_2_START = 2 * 256;
        private $T8_3_START = 3 * 256;
        private $T8_4_START = 4 * 256;
        private $T8_5_START = 5 * 256;
        private $T8_6_START = 6 * 256;
        private $T8_7_START = 7 * 256;
        ...
        public function update($b, $off, $len){
            $localCrc = $this->crc;
    
            while($len > 7){
                $c0 = ($b[$off + 0] ^ $localCrc) & 0xff;
                $c1 = ($b[$off + 1] ^ ($localCrc >>= 8)) & 0xff;
                $c2 = ($b[$off + 2] ^ ($localCrc >>= 8)) & 0xff;
                $c3 = ($b[$off + 3] ^ ($localCrc >>= 8)) & 0xff;
                $localCrc = ($this->T[$this->T8_7_START + $c0] ^ $this->T[$this->T8_6_START + $c1])
                        ^ ($this->T[$this->T8_5_START + $c2] ^ $this->T[$this->T8_4_START + $c3]);
                ...
                $localCrc ^= ($this->T[$this->T8_3_START + $c4] ^ $this->T[$this->T8_2_START + $c5])
                        ^ ($this->T[$this->T8_1_START + $c6] ^ $this->T[$this->T8_0_START + $c7]);
            ...
            switch($len){
                case 7:
                    $localCrc = ($localCrc >> 8) ^ $this->T[$this->T8_0_START + (($localCrc ^ $b[$off++]) & 0xff)];
                case 6:
                    $localCrc = ($localCrc >> 8) ^ $this->T[$this->T8_0_START + (($localCrc ^ $b[$off++]) & 0xff)];
                case 5:
                    $localCrc = ($localCrc >> 8) ^ $this->T[$this->T8_0_START + (($localCrc ^ $b[$off++]) & 0xff)];
