class CRC32C {
    ...
    static T8_0_START = 0 * 256;
    static T8_1_START = 1 * 256;
    static T8_2_START = 2 * 256;
    static T8_3_START = 3 * 256;
    static T8_4_START = 4 * 256;
    static T8_5_START = 5 * 256;
    static T8_6_START = 6 * 256;
    static T8_7_START = 7 * 256;
    ...
    update(b, off, len){
        if(off && len){
            ...
            while(len > 7){
                let c0 = (b[off + 0] ^ localCrc) & 0xff;
                let c1 = (b[off + 1] ^ (localCrc >>>= 8)) & 0xff;
                let c2 = (b[off + 2] ^ (localCrc >>>= 8)) & 0xff;
                let c3 = (b[off + 3] ^ (localCrc >>>= 8)) & 0xff;
                localCrc = (CRC32C.T[CRC32C.T8_7_START + c0] ^ CRC32C.T[CRC32C.T8_6_START + c1])
                        ^ (CRC32C.T[CRC32C.T8_5_START + c2] ^ CRC32C.T[CRC32C.T8_4_START + c3]);

                let c4 = b[off + 4] & 0xff;
                let c5 = b[off + 5] & 0xff;
                let c6 = b[off + 6] & 0xff;
                let c7 = b[off + 7] & 0xff;
                localCrc ^= (CRC32C.T[CRC32C.T8_3_START + c4] ^ CRC32C.T[CRC32C.T8_2_START + c5])
                        ^ (CRC32C.T[CRC32C.T8_1_START + c6] ^ CRC32C.T[CRC32C.T8_0_START + c7]);
            ...
            switch(len){
                case 7:
                    localCrc = (localCrc >>> 8) ^ CRC32C.T[CRC32C.T8_0_START + ((localCrc ^ b[off++]) & 0xff)];
                case 6:
                    localCrc = (localCrc >>> 8) ^ CRC32C.T[CRC32C.T8_0_START + ((localCrc ^ b[off++]) & 0xff)];
                case 5:
                    localCrc = (localCrc >>> 8) ^ CRC32C.T[CRC32C.T8_0_START + ((localCrc ^ b[off++]) & 0xff)];
            ...
            this.#crc = (this.#crc >>> 8) ^ CRC32C.T[CRC32C.T8_0_START + ((this.#crc ^ b) & 0xff)];
        }
    }
}
