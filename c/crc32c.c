int T8_0_START = 0 * 256;
int T8_1_START = 1 * 256;
int T8_2_START = 2 * 256;
int T8_3_START = 3 * 256;
int T8_4_START = 4 * 256;
int T8_5_START = 5 * 256;
int T8_6_START = 6 * 256;
int T8_7_START = 7 * 256;
...
struct crc32c_struct {
    unsigned int data;
} crc32c_init = { 0xffffffff };

typedef struct crc32c_struct crc32c;

void reset(crc32c* crc){
    crc->data = 0xffffffff;
}
...
void update(char* b, int off, int len, crc32c* crc){
    ...
    while(len > 7){
        ...
        int c2 = (b[off + 2] ^ (crc->data >>= 8)) & 0xff;
        int c3 = (b[off + 3] ^ (crc->data >>= 8)) & 0xff;
        crc->data = (T[T8_7_START + c0] ^ T[T8_6_START + c1])
        ...
        crc->data ^= (T[T8_3_START + c4] ^ T[T8_2_START + c5])
    ...
    switch(len){
        case 7:
            crc->data = (crc->data >> 8) ^ T[T8_0_START + ((crc->data ^ b[off++]) & 0xff)];
        case 6:
            crc->data = (crc->data >> 8) ^ T[T8_0_START + ((crc->data ^ b[off++]) & 0xff)];
        case 5:
            crc->data = (crc->data >> 8) ^ T[T8_0_START + ((crc->data ^ b[off++]) & 0xff)];
